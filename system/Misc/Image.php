...
class Image
{
    ...
    function load($filename) {
        ...
        if (!extension_loaded('gd')) {
            throw new BpException('Required extension GD is not loaded.');
        }
    ...
    function output($format = null, $quality = null) {
        ...
        switch ($mimetype) {
            ...
            case 'image/png':
                ...
                break;
            default:
                throw new BpException('Unsupported image format: '.$this->filename);
                break;
        }
    }
     ...
     *
     * @param string        $overlay        An image filename or a BabiPHP\Misc\Image object
    ...
    function save($filename = null, $quality = null, $format = null) {
        ...
        switch (strtolower($format)) {
            ...
            case 'png':
                ...
                break;
            default:
                throw new BpException('Unsupported format');
        }

        if (!$result) {
            throw new BpException('Unable to save image: ' . $filename);
        }
    ...
    function text($text, $font_file, $font_size = 12, $color = '#000000', $position = 'center', $x_offset = 0, $y_offset = 0) {
        ...
        if (!$box) {
            throw new BpException('Unable to load font: '.$font_file);
        }
        ...
        if(empty($this->imagestring)) {
            ...
            switch ($info['mime']) {
                ...
                case 'image/png':
                    ...
                    break;
                default:
                    throw new BpException('Invalid image: '.$this->filename);
                    break;
            }
        } elseif (function_exists('getimagesizefromstring')) {
            $info = getimagesizefromstring($this->imagestring);
        } else {
            throw new BpException('PHP 5.4 is required to use method getimagesizefromstring');
        }
